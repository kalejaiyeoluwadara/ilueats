...
/** Shimmering skeleton for one admin Stores-page store card. */
export function AdminStoreCardSkeleton() {
  return (
    <div className="rounded-[1.25rem] bg-[var(--color-surface)] p-5 shadow-crisp ring-1 ring-[var(--color-line)]">
      <div className="min-w-0">
        <div className="h-4 w-2/3 rounded bg-[var(--color-line)] skeleton" />
        <div className="mt-2 h-3 w-1/3 rounded bg-[var(--color-line)] skeleton" />
      </div>
      <div className="mt-4 flex items-center justify-between gap-3">
        <div className="h-6 w-16 rounded-full bg-[var(--color-line)] skeleton" />
        <div className="h-3.5 w-10 rounded bg-[var(--color-line)] skeleton" />
      </div>
      <div className="mt-4 grid grid-cols-2 gap-2">
        {Array.from({ length: 2 }).map((_, idx) => (
          <div
            key={idx}
            className="rounded-2xl bg-[var(--color-bg)] px-3 py-3 ring-1 ring-[var(--color-line)]"
          >
            <div className="h-3 w-16 rounded bg-[var(--color-line)] skeleton" />
            <div className="mt-2 h-5 w-12 rounded bg-[var(--color-line)] skeleton" />
          </div>
        ))}
      </div>
      <div className="mt-4 grid grid-cols-2 gap-2">
        {Array.from({ length: 4 }).map((_, idx) => (
          <div
            key={idx}
            className="h-10 rounded-full bg-[var(--color-line)] skeleton"
          />
        ))}
      </div>
      <div className="mt-3 h-3.5 w-28 mx-auto rounded bg-[var(--color-line)] skeleton" />
    </div>
  );
}

/** Grid of admin store-card skeletons matching the Stores page layout. */
export function AdminStoreGridSkeleton({ count = 6 }: { count?: number }) {
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      {Array.from({ length: count }).map((_, idx) => (
        <AdminStoreCardSkeleton key={idx} />
      ))}
    </div>
  );
}

/** Shimmering skeleton for one admin Items-page item card. */
export function AdminItemCardSkeleton() {
  return (
    <div className="rounded-[1.25rem] bg-[var(--color-surface)] p-4 shadow-crisp ring-1 ring-[var(--color-line)]">
      <div className="flex gap-3">
        <div className="h-16 w-16 shrink-0 rounded-xl bg-[var(--color-line)] skeleton" />
        <div className="min-w-0 flex-1">
          <div className="h-3.5 w-3/4 rounded bg-[var(--color-line)] skeleton" />
          <div className="mt-2 h-3 w-1/2 rounded bg-[var(--color-line)] skeleton" />
          <div className="mt-2.5 h-3.5 w-16 rounded bg-[var(--color-line)] skeleton" />
        </div>
      </div>
      <div className="mt-3.5 grid grid-cols-3 gap-2">
        {Array.from({ length: 3 }).map((_, idx) => (
          <div
            key={idx}
            className="h-9 rounded-full bg-[var(--color-line)] skeleton"
          />
        ))}
      </div>
    </div>
...
/** Grid of admin item-card skeletons matching the Items page layout. */
export function AdminItemGridSkeleton({ count = 6 }: { count?: number }) {
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      {Array.from({ length: count }).map((_, idx) => (
        <AdminItemCardSkeleton key={idx} />
      ))}
    </div>
  );
}
