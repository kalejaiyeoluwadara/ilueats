...
export function StoreCard({ store, index = 0, variant = "vertical" }: StoreCardProps) {
  ...
  return (
    ...
    >
      <Link
        href={`/${store.slug}`}
        className="group block focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)]/40 focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--color-bg)] rounded-2xl"
      >
        <div className="overflow-hidden rounded-2xl">
          ...
          >
            ...
            <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between gap-2">
              ...
              <span className="flex items-center gap-1 rounded-full bg-white/95 px-2 py-1 text-[11px] font-bold text-[var(--color-ink)]">
        ...
        <div className="px-1 pt-2.5">
          <p className="line-clamp-1 text-[13px] text-[var(--color-ink-muted)]">
            {store.tagline}
          </p>
          <div className="mt-1.5 flex items-center gap-3 text-[12px] font-medium text-[var(--color-ink-muted)]">
            <span className="inline-flex items-center gap-1">
              <ClockIcon className="h-3.5 w-3.5 text-[var(--color-ink-soft)]" />
              {formatDeliveryTime(store.deliveryTimeMins)}
            </span>
            <span aria-hidden className="text-[var(--color-ink-soft)]">·</span>
            <span className="inline-flex items-center gap-1">
              <TruckIcon className="h-3.5 w-3.5 text-[var(--color-ink-soft)]" />
